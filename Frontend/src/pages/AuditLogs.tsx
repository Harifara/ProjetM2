...
const AuditLogs = () => {
  ...
  const filteredLogs = logs.filter(log =>
    (log.user_info?.username || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="flex gap-4">
        ...
        <ExportButton endpoint="logs" data={filteredLogs} filename="audit-logs" />
      </div>
              ...
              {filteredLogs.map((log) => (
                <TableRow key={log.id}>
                  ...
                  <TableCell className="font-medium">
                    {log.user_info?.full_name || log.user_info?.username || 'Utilisateur inconnu'}
                  </TableCell>
