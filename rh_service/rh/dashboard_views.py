...
def dashboard_rh(request):
    ...
    Compatible avec Kong JWT (KongJWTUser)
    """
    user = request.user
    today = timezone.now().date()

    # 🔐 Vérification permissions adaptée à KongJWTUser
    # user.role ou user.roles selon la configuration Kong
    user_role = getattr(user, "role", None)  # si c'est un string
    user_roles = getattr(user, "roles", [])  # si c'est une liste
    roles_list = [user_role] if user_role else user_roles

    if "RH" not in roles_list and "ADMIN" not in roles_list:
    ...
    # ==============================
    charts = {
        "employes_par_sexe": list(Employer.objects.values("sexe").annotate(total=Count("id"))),
        "employes_par_statut": list(Employer.objects.values("status_employer").annotate(total=Count("id"))),
        "employes_par_fonction": list(
            Employer.objects.values("fonction__nom_fonction").annotate(total=Count("id")).order_by("-total")
        ),
        "employes_par_district": list(
            Employer.objects.values("district__name").annotate(total=Count("id")).order_by("-total")
        ),
        "conges_par_statut": list(Conge.objects.values("status_conge").annotate(total=Count("id"))),
        "contrats_par_nature": list(Contrat.objects.values("nature_contrat").annotate(total=Count("id"))),
        "demandes_par_statut": list(Demande.objects.values("status").annotate(total=Count("id"))),
    ...
    lists = {
        ...
        "affectations": AffectationSerializer(
            Affectation.objects.select_related("employer", "nouveau_fonction", "nouveau_district")
